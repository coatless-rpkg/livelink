...
Create shareable links for R code in WebAssembly (WASM) environments: webR for a
full R REPL, and Shinylive for R and Python Shiny apps.
...
never send anywhere, so the recipient clicks the link and lands in a running R
...
The examples below create and decode links for R code, R Shiny apps, and Python
Shiny apps.

### webR links

Share R code that runs in the browser. Pass the code straight in, with no quotes
and no escaping, just braces:
...
Write the `list()` inside the call. If you assign it to a variable first, R runs
the braces before livelink ever sees them.

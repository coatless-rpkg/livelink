...
paste the URL into another. Add `livelink: true` to an ordinary chunk and it keeps
running exactly as before — output, plots and all — with a link added underneath:
...
For code your session should *not* run — a Shiny app, or anything needing a package you
have not installed — use a ```` ```{livelink} ```` chunk instead: the code is shown, a
link is produced, and nothing is executed.

Either way, your comments survive into the link, which they do not when a `{ }`
expression is knitted. See [`?livelink-knitr`](../reference/livelink-knitr.html).

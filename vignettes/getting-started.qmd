...
- **Shinylive**: Run R and Python Shiny applications without requiring a server using [Shinylive](https://shiny.posit.co/py/get-started/shinylive.html)
...
analysis_link <- webr_repl_link(code, panels = c("editor", "plot"))

# Show all components for full development environment
full_link <- webr_repl_link(code, panels = c("editor", "plot", "files", "terminal"))

# Use string format for convenience
focused_link <- webr_repl_link(code, panels = "editor-plot")
...
set_webr_base_url("https://example.com/webr/")
...
`set_webr_base_url()` mutates a global option, so in a script or report prefer
scoping the change to a single block:

```{r}
withr::with_options(
  list(livelink.base_url = "https://example.com/webr/"),
  webr_repl_link(code)
)
```
...
Now that you understand the basics, the reference index documents every
function in detail:

- `webr_repl_project()` and `webr_repl_directory()` for multi-file projects and
  batch processing
- `webr_repl_exercise()` for paired exercise and solution links
- `shinylive_project()` and `shinylive_directory()` for multi-file Shiny apps
- `decode_webr_link()` and `preview_webr_link()` to go the other way, turning a
  link back into files

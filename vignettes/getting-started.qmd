...
![Your code is serialized to JSON, gzipped, base64-encoded, and placed in the fragment of a webR URL after the `#`. Opening the link runs it in the browser.](figures/pipeline-light.svg){.ll-fig .ll-light}
![](figures/pipeline-dark.svg){.ll-fig .ll-dark aria-hidden="true"}
...
![A webR URL in five parts: the base address, the version, the panels to show, the `#code=` fragment carrying your files, and the flags saying how the payload was encoded.](figures/url-anatomy-light.svg){.ll-fig .ll-light}
![](figures/url-anatomy-dark.svg){.ll-fig .ll-dark aria-hidden="true"}

...
![The four webR panels. `panels` selects which are shown.](../man/figures/livelink-panels.svg){fig-alt="A schematic browser window divided into four labelled regions: an editor pane for code, a plot pane for graphics, a files pane for the file browser, and a terminal pane for the R console."}
...
renders the code *and* a link to run it, with your comments intact.

This is the recommended way to embed links in a document, and not only for
convenience. `webr_repl_link({ ... })` recovers your code from R's source
...
needs them most. The chunk engine is handed the raw source, so nothing is lost.

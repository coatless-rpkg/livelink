...
in one chunk and paste the URL into another. livelink registers a **chunk engine**:
write the code as a normal chunk, get a link.
...
renders the code *and* a link to run it, with your comments intact. That chunk is a
real one — here it is, executed in this very vignette:
...
Follow the link and the comments are still there — which is the whole reason the
engine exists. `webr_repl_link({ ... })` recovers your code from R's source
references, and knitr discards those when it renders a chunk, so comments inside a
`{ }` expression silently vanish from the link. Exactly where a teaching example
needs them most.
...
and `engine.target` switches to Shinylive when you want an app instead of a REPL:

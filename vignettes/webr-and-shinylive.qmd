...
A **named list**, when the code is already in R. The names become the filenames:
...
Non-R files are welcome. Data, documentation, anything the code needs travels with it.
...
Only `.R` files can autorun. Listing a `.csv` is harmless — it simply is not run.
...
`R CMD check` scans documents for `library()` calls and would report shiny as an
undeclared dependency of livelink, which it is not.

Reach for braces when the code is R you want executed in webR. For a Shiny app, a
string or a file path says what you actually mean — and leaves the app's own quoting
alone.
:::

For an app split across files, pass a named list. `app.R` is the entry point; the rest
come along:
...
`header` does nothing in `editor` mode — there is no header to hide.
...
`webr_repl_directory()` is the webR counterpart, but it works the other way round: it
gives each R script in a directory *its own* link, rather than bundling a folder into
one. That suits a folder of course scripts, and
